...
export default function CustomTable<D extends DataRecord = DataRecord>(
  ...
  const getColumnConfigs = useCallback(
    (
      ...
      i: number,
      column_styles: Record<string, ColumnStyleData>,
      ...
      const hasColumnStyle = Boolean(column_styles[column.key]);
      ...
      return {
        ...
        Cell: ({ value, row }: { value: DataRecordValue; row: Row<D> }) => {
          ...
          if (hasColumnStyle) {
            const arrayLength = column_styles[column.key]?.backgroundColor?.length || 0;
            backgroundColor = column_styles[column.key]?.backgroundColor?.[row.index%arrayLength];
            color = column_styles[column.key]?.textColor;
          }

          if (hasTextFont) {
            sharedStyle.fontFamily = `${column_styles[column.key]?.textFont}, ${sharedStyle.fontFamily}`;
          }
          ...
          const StyledCell = styled.td`
            font-family: ${sharedStyle.fontFamily};
  ...
  const columns = useMemo(
    () => visibleColumnsMeta.map((column, i) => getColumnConfigs(column, i, props.column_styles)),
    [visibleColumnsMeta, getColumnConfigs, props.column_styles],
  );
